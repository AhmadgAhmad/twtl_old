...
from dfa import intersection, setDFAType, DFAType, Op, setOptimizationFlag
...
#                     kind=DFAType.Normal, norm=True)
    formula = '[H^2 A]^[0, 4] | [H^2 B]^[2, 5]'
    res = translate('[H^3 !A]^[0, 8] * [H^2 B & [H^4 C]^[3, 9]]^[2, 19]',
                    kind=DFAType.Infinity, norm=True)
    # ast_formula = AST_twtl(formula)
    traj = [1,2,3,4,5,6]
    rho = robustness(ast=res[1].tree,word=traj)

    # Debugging to codding 
    # f1,f2 = ast.left, ast.right # Union operation 
    print res
    print res[1].g.nodes()
